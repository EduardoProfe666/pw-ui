  ...
  <v-app-bar color="primary" elevation="1">
    ...
    <v-menu location="bottom end" transition="scale-transition">
      ...
      <v-list width="300" elevation="1">
        ...
        <v-divider></v-divider>
        <v-list-item to="/profile" prepend-icon="mdi-account-circle" title="My Profile"></v-list-item>
        <v-list-item to="/profile/settings" prepend-icon="mdi-cog" title="Settings"></v-list-item>
        <v-divider></v-divider>
        <v-list-item @click="authStore.logout" prepend-icon="mdi-logout" title="Logout" color="error"></v-list-item>
      </v-list>
    </v-menu>
  </v-app-bar>
  ...
  <v-navigation-drawer v-model="drawer" elevation="1">
    ...
    <v-list density="compact" nav>
      ...
      <template v-if="isAdmin">
        ...
        <v-list-item to="/students" prepend-icon="mdi-school" title="Students" rounded="lg" />
        <v-list-item to="/evaluations" prepend-icon="mdi-clipboard-text" title="Evaluations" rounded="lg" />
      </template>
    </v-list>
  </v-navigation-drawer>
</template>
